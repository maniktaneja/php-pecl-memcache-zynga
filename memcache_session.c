...
PS_READ_FUNC(memcache)
{
	...
	if (pool) {
		...
		if (mmc_exec_retrieval_cmd(pool, key_tmp, key_tmp_len, &result, NULL, NULL, 0 TSRMLS_CC) <= 0 || Z_TYPE_P(result) != IS_STRING) {
...
PS_WRITE_FUNC(memcache)
{
	...
	if (pool) {
		...
		if (mmc_pool_store_wrapper(pool, "set", sizeof("set")-1, key_tmp, key_tmp_len, 0, INI_INT("session.gc_maxlifetime"), 0, val, vallen, 0, NULL, 0, 0 TSRMLS_CC)) {
			return SUCCESS;
		}
	}
